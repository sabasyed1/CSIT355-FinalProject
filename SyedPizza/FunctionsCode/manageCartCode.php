...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $numExistRows = checkCartItemForUser($itemId,$userId);
        if($numExistRows > 0){
            echo "<script>
            ...
            $sql = "INSERT INTO `cart` (`productId`, `quantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['checkout'])) {
        ...
        if (password_verify($password, $passRow['password'])){ 
            $sql = "INSERT INTO `orders` (`userId`, `address`, `zipCode`, `phoneNo`, `amount`, `payment`, `orderStatus`, `orderDate`) VALUES ('$userId', '$address', '$zipcode', '$phone', '$amount', '0', '0', current_timestamp())";
            $result = mysqli_query($conn, $sql);
            ...
            if ($result){
                ...
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                    window.location.href="http://localhost/OnlinePizzeria/index.php";  
                    </script>';
                    exit();
            }
        } 
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        ...
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `cart` SET `itemQuantity`='$qty' WHERE `productId`='$productId' AND `userId`='$userId'";
